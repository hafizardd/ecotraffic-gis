...
class SurveyStopObservation(Base):
    ...
    accessibility_score_100: Mapped[float | None] = mapped_column(Float, nullable=True)
    condition_score_100: Mapped[float | None] = mapped_column(Float, nullable=True)
    environment_score_100: Mapped[float | None] = mapped_column(Float, nullable=True)
    ahp_total_score: Mapped[float | None] = mapped_column(Float, nullable=True)
    ahp_rank: Mapped[int | None] = mapped_column(Integer, nullable=True)
    ahp_classification: Mapped[str | None] = mapped_column(String(40), nullable=True)
    ahp_weight_version: Mapped[str | None] = mapped_column(String(40), nullable=True)
    facility_checklist: Mapped[dict | None] = mapped_column(JSONB, nullable=True)
    damage_indicators: Mapped[dict | None] = mapped_column(JSONB, nullable=True)
    poi_breakdown_survey: Mapped[dict | None] = mapped_column(JSONB, nullable=True)

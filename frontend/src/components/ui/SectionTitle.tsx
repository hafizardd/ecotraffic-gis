        ...
        <div className={`mb-[14px] flex min-w-0 flex-col gap-[5px] ${page ? "mb-5" : ""}${className ? ` ${className}` : ""}`}>
            <div className="flex items-center justify-between gap-3">
                <div className="flex min-w-0 flex-col gap-[5px]">
                    ...
                    <Heading className={`m-0 font-(family-name:--font-display) font-semibold leading-[1.25] tracking-[-0.02em] text-(--text) ${page ? "text-(--text-section)! leading-[1.2]!" : "text-(--text-card-title)"}`}>{title}</Heading>
                </div>
                {aside && <span className="flex-[0_0_auto] text-right text-(--text-meta) font-semibold text-(--secondary) tabular-nums [&>div]:min-w-[150px] [&>div]:w-auto">{aside}</span>}
            </div>

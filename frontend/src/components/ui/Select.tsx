...
export default function Select({ value, options, onChange, ariaLabel, placeholder = "Pilih", className,
    ...
    return (
        <div className={`relative w-full${className ? ` ${className}` : ""}`} ref={rootRef}>
            <button type="button" className="group flex min-h-[var(--control-height)] w-full cursor-pointer items-center justify-between gap-[var(--space-2)] rounded-[var(--radius-sm)] border border-[var(--contour-strong)] bg-[var(--surface-raised)] px-[var(--space-3)] text-left text-xs font-[var(--weight-label)] text-[var(--text)] transition-colors duration-150 hover:border-[var(--selection)] hover:bg-[var(--surface-hover)] aria-expanded:border-[var(--selection)]" role="combobox" aria-haspopup="listbox"

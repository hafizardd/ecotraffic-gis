...
export default function HistoryPagination({ displayedCount, total, pageSize, onPageSizeChange, page, totalPages, onPageChange, loading, hasData }: HistoryPaginationProps) {
    ...
    return <nav className="my-3.5 flex items-center justify-between gap-4 text-xs text-[#94a3b8] max-[600px]:flex-row max-[600px]:flex-nowrap max-[600px]:gap-2" aria-label="Navigasi halaman riwayat">
        <div className="relative flex min-w-0 flex-wrap items-center gap-2.5 max-[600px]:flex-nowrap max-[600px]:gap-2" ref={rootRef}>
            <span className="inline-flex items-center gap-1.25">
                Menampilkan
                <button ref={triggerRef} type="button" role="combobox" className="inline-flex min-h-10 cursor-pointer items-center gap-0.75 border-0 border-b border-dotted border-current bg-transparent px-1 font-[inherit] text-(--text) tabular-nums hover:text-(--green) focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-(--green)" aria-haspopup="listbox" aria-expanded={open}
            ...
            {open && <div className="absolute top-[calc(100%+6px)] left-0 z-20 max-h-70 min-w-30 overflow-y-auto rounded-sm border border-[#334155] bg-[#0e1d2e] p-(--space-1) shadow-[0_14px_34px_rgba(0,0,0,0.4)] animate-[select-in_0.14s_ease-out] motion-reduce:animate-none">
                <ul role="listbox" id={listId} aria-label="Jumlah baris per halaman" className="m-0 list-none p-0">
                    {PAGE_SIZE_OPTIONS.map((size, index) => <li id={`${listId}-option-${index}`} key={size} role="option" aria-selected={size === pageSize}>
                        <button type="button" tabIndex={-1}
                            className={`flex w-full cursor-pointer items-center justify-between gap-(--space-2) rounded-[5px] border-0 bg-transparent px-(--space-3) py-2.25 text-left text-xs text-(--secondary) [&>svg]:h-3.5 [&>svg]:w-3.5 ${index === active ? "bg-[#14283c] text-(--text)" : ""}${size === pageSize ? " font-(--weight-strong) text-(--green)" : ""}`}
        ...
        <div className="flex min-w-0 flex-wrap items-center gap-2.5 tabular-nums max-[600px]:flex-nowrap max-[600px]:gap-2">
            <span className="inline-flex items-center gap-1.25">

...
export default function AnalyticsMetricLedger({
    ...
    return (
        <section
            aria-label={label}
            aria-busy={loading}
            className="mb-3.5 overflow-hidden rounded-md border border-(--border) bg-(--card)"
        >
            <dl className="grid grid-cols-[repeat(auto-fit,minmax(116px,1fr))]">
                {items.map((item) => (
                    <div
                        key={item.key}
                        className={`relative flex min-h-23.5 min-w-0 flex-col justify-center border-r border-b border-(--border) px-3 py-3.25 last:border-r-0 ${item.primary ? "bg-(--selection-soft)" : ""}`}
                    >
                        <i className="absolute top-0 left-3 h-0.75 w-7" style={{ backgroundColor: item.color ?? "var(--contour-strong)" }} aria-hidden="true" />
                        {loading ? <>
                            ...
                            <dd className="mt-1.75 mb-0 text-[10px] leading-[1.35] text-(--muted)">{item.unit}{item.meta ? <> · {item.meta}</> : null}</dd>
                        </>}
                    </div>
                ))}
            </dl>

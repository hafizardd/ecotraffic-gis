...
export default function AnalyticsMeasureBand({
    ...
    return (
        <section
            aria-label={label}
            className="mb-3.5 overflow-hidden rounded-md border border-(--border) bg-(--surface-sunken)"
        >
            ...
            <dl className="grid grid-cols-[repeat(auto-fit,minmax(118px,1fr))]">
                {items.map((item, index) => (
                    <div
                        ...
                        className="relative min-w-0 border-r border-(--border) px-3 pt-2.75 pb-3 last:border-r-0"
                    >
                        <span
                            className={`absolute top-0 left-3 h-0.75 w-7 ${TONE_CLASS[item.tone ?? "default"]}`}
                            aria-hidden="true"
                        />

...
const FILTERS_CLASS = "flex flex-wrap items-end gap-3.5 [&>label]:flex [&>label]:flex-[1_1_160px] [&>label]:flex-col [&>label]:gap-1.75 [&>label]:text-xs [&>label]:text-(--secondary) [&>label>div]:max-w-85";
const DATE_INPUT_CLASS = "min-h-(--control-height) w-full max-w-85 rounded-sm border border-[#334155] bg-[#102238] p-(--space-2) text-[#edf5ff] [color-scheme:dark]";
...
export default function AnalyticsFilters() {
    ...
    return <section className="mb-3.5 rounded-md border border-(--border) bg-(--surface-sunken)">
        ...
        <details className="mt-3 border-t border-(--border) pt-2 text-xs text-(--secondary)"><summary className="cursor-pointer py-1.75 font-semibold">Gunakan rentang tanggal & waktu</summary><form action={applyDates} className={`${FILTERS_CLASS} pt-2`}>

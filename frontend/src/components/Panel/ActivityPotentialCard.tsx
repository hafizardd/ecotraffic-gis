...
import { fmtFloatId, fmtIntId } from "@/utils/format";
    ...
    return (
        <>
            <div className="segment-overview">
                <strong>{properties.klasifikasi_potensi}</strong>
                <span>Peringkat {fmtIntId(properties.ranking)} dari {TOTAL_HEXES}</span>
                <b className="priority-badge" style={{ background: badgeColor, color: badgeText }}>{properties.klasifikasi_potensi}</b>
            </div>
            ...
            <section className="panel-section">
                ...
                {Object.entries(INPUT_LABELS).map(([key, label]) => {
                    const value = Number(properties[key as keyof typeof properties] ?? 0);

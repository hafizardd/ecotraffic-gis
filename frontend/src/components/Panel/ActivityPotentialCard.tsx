    ...
    return (
        <>
            ...
            {properties.data_status === "fallback" && (
                <p className="analytics-note">Perkiraan dari area terdekat; tidak ada data kendaraan langsung untuk jam ini.</p>
            )}
            <section className="panel-section segment-decision-section">
                <SectionTitle title="Skor potensi" />
                <div className="segment-score"><span>SKOR TOTAL</span><strong>{fmtFloatId(properties.skor_total_ahp, 2)}</strong></div>
            </section>

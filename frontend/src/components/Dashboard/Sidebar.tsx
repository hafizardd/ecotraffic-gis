...
export default function Sidebar({ open, mobileOpen, onToggle, onMobileClose, activeView, onViewChange }: SidebarProps) {
    ...
    return (
        ...
        <aside ref={sidebarRef} id="primary-navigation" role={mobileOpen ? "dialog" : undefined} aria-modal={mobileOpen ? true : undefined} aria-label="Navigasi aplikasi" className={`z-[60] flex w-[216px] flex-[0_0_216px] flex-col overflow-hidden border-r border-[var(--border)] bg-[var(--sidebar)] transition-[width,flex-basis,transform,visibility] duration-200 ease-out max-[1100px]:w-[68px] max-[1100px]:basis-[68px] max-[760px]:fixed max-[760px]:inset-y-0 max-[760px]:left-0 max-[760px]:w-[216px] max-[760px]:basis-[216px] max-[760px]:shadow-[var(--shadow-float)] ${open ? "" : "w-[68px] basis-[68px]"} ${mobileOpen ? "max-[760px]:visible max-[760px]:translate-x-0" : "max-[760px]:invisible max-[760px]:-translate-x-full"}`}>
            <div className="flex h-14 flex-[0_0_56px] items-center gap-3 whitespace-nowrap border-b border-[var(--border)] px-4 max-[1100px]:px-[15px] max-[760px]:px-4">
                ...
                <div className={`flex min-w-0 items-center transition-opacity duration-150 ${copyVisibility}`}><strong className="font-[var(--font-display)] text-sm font-bold tracking-[-0.01em]">EcoTraffic GIS</strong></div>
                <button type="button" className="ml-auto hidden h-11 w-11 place-items-center rounded-[var(--radius-sm)] border border-[var(--border)] bg-[var(--surface)] text-[var(--secondary)] hover:border-[var(--contour-strong)] hover:text-[var(--text)] max-[760px]:grid" aria-label="Tutup menu" onClick={closeMobileAndRestoreFocus}><X className="h-[17px] w-[17px]" aria-hidden="true" /></button>
            </div>
            <nav className="flex flex-col gap-1 px-3 py-4" aria-label="Navigasi utama">
                {navItems.map((item) => (
                    <button key={item.label} type="button" className={`flex min-h-10 cursor-pointer items-center gap-3 whitespace-nowrap rounded-[var(--radius-sm)] border border-transparent bg-transparent px-3 text-[var(--text-muted)] transition-[color,background,border-color] duration-150 ease-out hover:bg-[var(--surface)] hover:text-[var(--text)] max-[760px]:min-h-11 ${activeView === item.view ? "border-[rgba(34,197,94,0.2)]! bg-[var(--brand-soft)]! text-[var(--brand-strong)]! shadow-[inset_3px_0_var(--brand)]" : ""}`} title={item.label} aria-current={activeView === item.view ? "page" : undefined} onClick={() => onViewChange(item.view)}>

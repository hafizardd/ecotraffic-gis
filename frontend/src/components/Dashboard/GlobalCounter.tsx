...
export default function GlobalCounter() {
    ...
    const metrics = EMISSION_DEFINITIONS.map(({ key, label }) => (
        <div key={key} className={`grid min-w-[92px] grid-rows-[auto_auto_auto] content-center gap-y-0.5 border-r border-(--border) px-3 last:border-0 ${POLLUTANT_TEXT_CLASS[key]}`}>
            {hasData ? <>
    ...
    return (
        <>
            <section className="flex min-h-[60px] min-w-0 items-stretch border-b border-(--border) bg-(--surface-sunken) py-2 pr-3 pl-4 max-[760px]:hidden" aria-label="Ringkasan emisi kota">
                <div className="flex w-[142px] flex-[0_0_142px] flex-col justify-center border-r border-(--border) max-[1100px]:w-[124px] max-[1100px]:basis-[124px]"><span className="text-[10px] font-bold tracking-[0.14em] text-(--brand-strong) uppercase">Telemetri kota</span><strong className="mt-0.5 text-[12px] font-semibold">8 polutan · saat ini</strong></div>

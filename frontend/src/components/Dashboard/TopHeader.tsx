...
export default function TopHeader({ onMenuClick, section, title }: { onMenuClick: () => void; section: string; title: string }) {
    ...
    const connectionColor = connectionStatus === "connected" ? "text-[var(--brand-strong)]" : connectionStatus === "connecting" ? "text-[var(--accent)]" : "text-[var(--danger)]";
    ...
    return (
        <header className="flex min-w-0 items-center gap-4 border-b border-[var(--border)] bg-[var(--shell)] px-5 max-[760px]:gap-3 max-[760px]:px-3">
            ...
            <div className={`flex items-center gap-2 text-[10px] font-bold tracking-[0.1em] uppercase max-[760px]:hidden ${connectionColor}`} role="status" aria-label={`Koneksi data ${connectionLabel}`}><span className="h-[7px] w-[7px] rounded-full bg-current shadow-[0_0_0_4px_rgba(34,197,94,0.1)]" aria-hidden="true" /> Koneksi {connectionLabel}</div>
            <div className="flex min-w-[118px] flex-col border-l border-[var(--border)] pl-4 text-right max-[760px]:min-w-0 max-[760px]:border-0 max-[760px]:pl-0">
                <strong className="font-[var(--font-data)] text-[13px] leading-4 tabular-nums">{now ? now.toLocaleTimeString("id-ID", { hour: "2-digit", minute: "2-digit", second: "2-digit" }) : "--:--:--"}</strong>
                <span className="text-[10px] leading-4 text-[var(--muted)] max-[760px]:hidden">{lastMessageAt ? `Data ${new Date(lastMessageAt).toLocaleTimeString("id-ID", { hour: "2-digit", minute: "2-digit", second: "2-digit" })}` : now ? now.toLocaleDateString("id-ID", { weekday: "short", day: "2-digit", month: "short", year: "numeric" }) : "Memuat waktu"}</span>
            </div>
        </header>
    );
}

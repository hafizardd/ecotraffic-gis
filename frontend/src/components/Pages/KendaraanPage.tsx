...
export default function KendaraanPage() {
    ...
    return <div className={`${PAGE_CONTAINER_CLASS} [&_:focus-visible]:outline-2 [&_:focus-visible]:outline-offset-3 [&_:focus-visible]:outline-[#22c55e]`}>
        ...
        {error && !data ? <p role="alert" className={ANALYTICS_ERROR_CLASS}>{error}</p> : resolving ? <>
            ...
            <div className="mb-[14px] flex flex-wrap items-center gap-x-3 border-y border-[var(--border)] bg-[var(--surface-sunken)] px-3 py-1.5">
                <span className="text-[9px] font-bold tracking-[0.1em] text-[var(--muted)] uppercase">Seri ditampilkan</span>
            ...
            <section className={`${PAGE_CARD_CLASS} ${ANIMATE_IN_CLASS}`} aria-label="Peringkat segmen" aria-busy={loading}>
                ...
                <div className={TABLE_WRAP_CLASS}><table className={PRIORITY_TABLE_CLASS}><thead><tr>
                    ...
                    <td><strong>{row.segment_name}</strong><br /><small>{row.corridor_name}</small><br />
                        <button type="button" className={`mt-1 cursor-pointer rounded-full border px-2 py-[3px] text-[10px] font-bold transition-colors duration-160 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[var(--green)] ${filter.segmentId === row.segment_id ? "border-[var(--green)] bg-[var(--green)] text-[#062018]" : "border-[var(--border)] bg-transparent text-[var(--green)] hover:border-[var(--green)] hover:text-[#86efac]"}`}
                    ...
                    <td className="text-right tabular-nums"><div className="relative min-w-[104px] overflow-hidden rounded-[var(--radius-badge)] bg-[var(--surface-sunken)] px-2 py-1"><span className="absolute inset-y-0 left-0 bg-[var(--selection)] opacity-[0.18]" style={{ width: `${maxRankingTotal > 0 ? ((row.total_veh_h ?? 0) / maxRankingTotal) * 100 : 0}%` }} aria-hidden="true" /><strong className="relative">{fmtIntId(row.total_veh_h)}</strong></div></td>
                </tr>)}</tbody></table></div>
            </section>

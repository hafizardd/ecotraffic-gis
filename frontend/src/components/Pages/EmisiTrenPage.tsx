...
export default function EmisiTrenPage() {
    ...
    return <div className={`${PAGE_CONTAINER_CLASS} [&_:focus-visible]:outline-2 [&_:focus-visible]:outline-offset-3 [&_:focus-visible]:outline-[#22c55e]`}>
        ...
        <section className={`${PAGE_CARD_CLASS} ${ANIMATE_IN_CLASS}`} aria-busy={loading} aria-label="Tren emisi">
            ...
            <div className="mb-[14px] flex flex-wrap items-center gap-2 border-b border-[var(--border)] pb-3 text-xs">
                ...
                <div className="relative">
                    {resolving ? <ChartSkeleton bars={5} height={250} />
                        : hasTop ? <>
                            ...
                            <div className={TABLE_WRAP_CLASS}><table className={PRIORITY_TABLE_CLASS}><thead><tr><th>#</th><th>Koridor / segmen</th><th>{rankingLabel} · kg/hour</th><th>Sampel</th><th>Teramati</th></tr></thead><tbody>{topData.map((row) => <tr key={row.corridor_id}><td>{row.rank}</td><td>{row.corridor_name}<br /><small>{row.segment_ids.join(", ")}{row.estimated_sample_count > 0 ? " · estimated" : ""}</small></td><td><div className="relative min-w-[106px] overflow-hidden rounded-[var(--radius-badge)] bg-[var(--surface-sunken)] px-2 py-1 text-right"><span className="absolute inset-y-0 left-0 opacity-20" style={{ width: `${maxRankEmission > 0 ? ((row.emission_kg_h ?? 0) / maxRankEmission) * 100 : 0}%`, backgroundColor: rankingColor }} aria-hidden="true" /><strong className="relative">{row.emission_kg_h == null ? "-" : formatRate(row.emission_kg_h)}</strong></div></td><td>{row.sample_count}</td><td>{fmtDateTimeId(row.observed_at)}</td></tr>)}</tbody></table></div>
                ...
                <div className="relative">
                    {resolving ? <ChartSkeleton bars={6} height={250} />
                        : hasComposition ? <>
                            ...
                            <div className="grid grid-cols-2 gap-x-5 gap-y-[7px] border-t border-[var(--border)] pt-3 max-[600px]:grid-cols-1">{data?.composition.data.map((p) => <div className="flex justify-between gap-2 text-xs" key={p.key}><button type="button" className="flex w-full cursor-pointer items-center justify-between gap-2 rounded-[5px] border-0 bg-transparent px-1 py-[5px] text-xs text-inherit hover:bg-[var(--surface-raised)] focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[var(--selection)] aria-pressed:opacity-100 aria-[pressed=false]:opacity-45 [&_i]:mr-[7px] [&_i]:inline-block [&_i]:h-[3px] [&_i]:w-3" aria-pressed={selected.includes(p.key)} onClick={() => toggle(p.key)}><span><i style={{ background: EMISSION_DEFINITIONS.find((d) => d.key === p.key)?.color }} />{p.pollutant}</span><strong className="font-[var(--font-data)] tabular-nums">{p.kg_h == null ? "-" : formatRate(p.kg_h)}</strong></button></div>)}</div>
                        </> : emptyState(resolving ? "Memuat analitik…" : status, !!error && !loading)}

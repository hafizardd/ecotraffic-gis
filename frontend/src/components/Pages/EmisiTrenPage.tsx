...
const updateOverlay = (loading: boolean, hasData: boolean) => loading && hasData
    ? <div className="absolute top-1 right-1.5 inline-flex items-center gap-(--space-2) rounded-full border border-(--border) bg-[rgba(7,20,34,0.82)] px-2.25 py-1 text-[11px] text-(--secondary)"><span className="h-3 w-3 animate-[spin_0.8s_linear_infinite] rounded-full border-[1.5px] border-[rgba(148,163,184,0.22)] border-t-(--green)" />Memperbarui…</div> : null;
...
export default function EmisiTrenPage() {
    ...
    return <div className={`${PAGE_CONTAINER_CLASS} [&_:focus-visible]:outline-2 [&_:focus-visible]:outline-offset-3 [&_:focus-visible]:outline-[#22c55e]`}>
        ...
        <section className={`${PAGE_CARD_CLASS} ${ANIMATE_IN_CLASS}`} aria-busy={loading} aria-label="Tren emisi">
            ...
            <div className="mb-3.5 flex flex-wrap items-center gap-2 border-b border-(--border) pb-3 text-xs">
                ...
                <div className="relative">
                    {resolving ? <ChartSkeleton bars={6} height={250} />
                        : hasComposition ? <>
                            ...
                            <div className="grid grid-cols-2 gap-x-5 gap-y-1.75 border-t border-(--border) pt-3 max-[600px]:grid-cols-1">{data?.composition.data.map((p) => <div className="flex justify-between gap-2 text-xs" key={p.key}><button type="button" className="flex w-full cursor-pointer items-center justify-between gap-2 rounded-[5px] border-0 bg-transparent px-1 py-1.25 text-xs text-inherit hover:bg-(--surface-raised) focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-(--selection) aria-pressed:opacity-100 aria-[pressed=false]:opacity-45 [&_i]:mr-1.75 [&_i]:inline-block [&_i]:h-0.75 [&_i]:w-3" aria-pressed={selected.includes(p.key)} onClick={() => toggle(p.key)}><span><i style={{ background: EMISSION_DEFINITIONS.find((d) => d.key === p.key)?.color }} />{p.pollutant}</span><strong className="font-(family-name:--font-data) tabular-nums">{p.kg_h == null ? "-" : formatRate(p.kg_h)}</strong></button></div>)}</div>
                        </> : emptyState(resolving ? "Memuat analitik…" : status, !!error && !loading)}
